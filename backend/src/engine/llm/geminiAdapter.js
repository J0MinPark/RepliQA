...
async function generateNextAction({
  ...
  // 성균관대 QA 실측 재현: 목표가 "kingo ID login 클릭"처럼 좁은 단일 행동이었는데도,
  // 그 버튼을 눌러 로그인 폼이 열리자 모델이 "목표를 달성한다"는 걸 "실제로 로그인까지
  // 완료한다"로 확대 해석해서 (아직 안 받은 ID/PW 없이) 로그인 버튼을 한 번 더 눌러버렸다.
  // 이건 예산 부족이 아니라 체크포인트 경계를 넘어선 것 — 다음 체크포인트(실제 ID/PW
  // 입력)의 몫을 미리 침범한 셈이라, 이후 체크포인트가 이미 잘못된 상태에서 시작하게 된다.
  const goalBlock = checkpointGoal
    ? `\n너의 현재 목표(체크포인트): "${checkpointGoal}"\n목표와 무관한 행동은 하지 마라. 이 목표는 대부분 아주 좁고 구체적인 단일 행동이다 —
문장에 적힌 행동을 글자 그대로 완료했다면, 그 다음에 자연스럽게 이어질 것 같은 행동(예:
로그인 버튼을 눌러 로그인 폼을 연 뒤, 목표엔 없는데도 실제 아이디/비밀번호를 입력하고
제출까지 시도하는 것)을 먼저 하지 마라 — 그건 다른 체크포인트의 몫일 수 있고, 특히 아직
받지 않은 정보(아이디/비밀번호 등)가 필요한 행동이면 절대 하지 마라. 목표 문장에 적힌
행동을 완료한 그 순간 바로 finish(success)로 멈춰라. finish로 멈출 때는 반드시
finishReason으로 성공/실패를 구분해라.

목표가 "OO 클릭"처럼 특정 요소를 클릭하는 것이었는데, 바로 직전 행동(action_history의
가장 최근 항목)이 정확히 그 요소를 클릭했고 result가 "success"였다면 — 그 클릭 자체가
이미 목표 달성이다. 클릭한 요소가 그 다음 화면에서 더 이상 안 보이거나 사라졌다고 해서
그걸 실패나 "요소를 못 찾음"의 증거로 오인하지 마라 — 오히려 클릭이 실제로 반영돼서
다른 화면/상태로 넘어갔다는 좋은 신호일 수 있다. 이 경우 화면이 바뀐 이유를 다시
찾으려 하지 말고 곧바로 finish(goal_achieved)로 멈춰라.

목표가 "~하면서 …을 확인"처럼 페이지를 훑어보며 여러 내용을 눈으로 검토하는 것이라면(특정
요소 하나를 클릭/입력하는 게 아니라 콘텐츠를 확인하는 것이라면): 실사이트(yureka.co.kr)
재현 실측 — "아직 페이지 하단에 많은 내용이 남아있는 것으로 보입니다" 같은 상투적인 문구만
반복하며 8번 넘게 스크롤만 하다가, 정작 목표에 해당하는 내용을 이미 화면에서 지나쳤는데도
그걸 못 알아채고 페이지 끝에서 "목표 내용을 찾지 못했다"며 실패 처리한 사례가 실제로
있었다. 이런 실수를 피하려면: 매 스텝 thought에 "아직 남아있다"류 상투어만 쓰지 말고, 지금
스크린샷에 실제로 보이는 구체적인 섹션 제목이나 문장을 짧게 적어라(예: "지금 'MENTORS'
섹션과 멘토 157명 소개가 보임"). 그리고 목표 문구에 적힌 단어가 사이트에 토씨 하나 안
틀리고 그대로 나올 거라 기대하지 마라 — 화면에 보이는 내용이 목표가 말하는 주제와 의미상
같다면(예: "자기주도 학습 기록" 섹션 ≈ "학습 관리 방식") 이미 확인된 것으로 인정해라. 목표가
말한 내용들을 화면에서 실제로 봤다면 페이지 끝까지 스크롤하지 않았어도 그 시점에 바로
finish(goal_achieved)로 마쳐라 — 페이지 맨 아래에 도달하는 것 자체가 목표가 아니다.\n`
    : '';
...
interactive_elements 목록의 요소에 "coversMostOfScreen": true가 붙어 있으면, 화면 절반
이상을 덮는 요소라는 뜻이다(직접 box의 width/height를 비교해서 계산할 필요 없음 — 엔진이
미리 계산해서 붙여준 값). 진짜 콘텐츠보다는 모달 백드롭·로딩 오버레이일 가능성이 높다.

화면에 뜬 팝업/모달을 닫는 게 목표인데, 목록에 "닫기"류 이름을 가진 요소가 여러 개 보여
헷갈릴 수 있다 — 예를 들어 모달 안의 작고 눈에 보이는 "닫기" 버튼과, coversMostOfScreen이
true인 어두운 배경 자체가 클릭하면 닫히는 큰 버튼(예: aria-label이 "팝업 닫기")이 함께
목록에 있을 수 있다. 이럴 때 "정확히 뭘 가리키는지 확신이 안 선다"는 이유로 finish(blocked)로
포기하지 마라 — 먼저 coversMostOfScreen이 없는(=작고 구체적인) 쪽을 시도하고, 그것도 없다면
coversMostOfScreen인 배경 버튼을 클릭하는 것도 정상적인 닫기 방법이다(모달 바깥을 클릭해서
닫는 흔한 UI 패턴).
...
//
// 중요: 이 함수는 runCheckpoint()에서 그 체크포인트의 행동 루프가 시작되기 전, "이 화면에
// 막 도착한 시점"에 단 한 번 호출된다 — 즉 screenshotBase64는 checkpointGoal에 적힌 행동을
// "아직 하기 전"의 화면이다. saucedemo.com 실측 재현: "사용자는 지금 '장바구니 아이콘을
// 클릭해서 담긴 상품을 확인' 단계에 있다"는 문구만으로 모델이 이걸 "그 행동을 이미
// 시도했는데 실패한 상황"으로 착각해서, 아직 클릭도 안 한 상품 목록 화면을 보고 "장바구니를
// 확인하려 했지만 상품 목록이 그대로 보인다"는 오탐을 냈다 — 이 함수는 화면의 시각적 품질만
...
카테고리를 다 채우려 하지 마라. 확신이 서지 않으면(이게 진짜 문제인지 스타일 취향인지
...
category는 반드시 아래 5개 영어 단어 중 정확히 하나와 완전히 똑같아야 한다 — 대소문자까지
그대로, 공백을 추가하거나("visual hierarchy") 다른 단어를 새로 만들지("copy" 등) 마라:
consistency, layout, feedback, typography, hierarchy
{
  "findings": [
