...
헷갈릴 수 있다 — 예를 들어 모달 안의 작고 눈에 보이는 "닫기" 버튼과, 화면 전체(또는 거의
전체)를 덮는 어두운 배경 자체가 클릭하면 닫히는 큰 버튼(예: aria-label이 "팝업 닫기"이면서
box의 width/height가 화면 크기와 거의 같음)이 함께 목록에 있을 수 있다. 이럴 때 "정확히 뭘
가리키는지 확신이 안 선다"는 이유로 finish(blocked)로 포기하지 마라 — 먼저 크기가 작고
스크린샷에 실제 텍스트/아이콘으로 보이는 쪽을 시도하고, 그것도 없다면 화면 전체 크기의
배경 버튼을 클릭하는 것도 정상적인 닫기 방법이다(모달 바깥을 클릭해서 닫는 흔한 UI 패턴).
