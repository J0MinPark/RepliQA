...
const IRREVERSIBLE_ACTION_KEYWORDS = [
  '회원 탈퇴',
  '계정 삭제',
  '영구 삭제',
  '완전히 삭제',
...
function isIrreversibleActionElement(text) {
  const normalized = normalize(text);
  if (!normalized) return false;
  return IRREVERSIBLE_ACTION_KEYWORDS.some((keyword) => normalized.includes(keyword.toLowerCase()));
}

module.exports = {
  isPaymentSubmitElement,
  isIrreversibleActionElement,
  PAYMENT_SUBMIT_KEYWORDS,
